...
export default Ember.Controller.extend({
  ...
  actions: {
    ...
    dayClicked: function(date, jsEvent, view) {
      ...
      if (!this.get('editingEventDetails')) { this.set('selectedDay', date); }
    },
    eventClicked: function(event, jsEvent, view) {
      // takes the user to the correct day on the agenda unless they are editing an event
      if (!this.get('editingEventDetails')) { this.set('selectedDay', event.start); }
    },   
    ...
    deleteEvent: function(event) {
      var events = this.get('events').filter(e => { return e.id !== event.id; });
